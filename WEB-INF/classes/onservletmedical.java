...
public class onservletmedical extends HttpServlet {
 ...
 protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
  ...
  {   
      ...
      String medicines[]=request.getParameterValues("txtbox[]");
       String quantity[]=request.getParameterValues("txtbox1[]");
  PreparedStatement ps=cn.prepareStatement("insert into mbill(mbid,mpid) values(?,?)");
       ps.setInt(1,b);
       ps.setInt(2,a);
    pw.println("updated");
  ...
  PreparedStatement ps1=cn.prepareStatement("insert into medbill(medbid,medid,quantity) values(?,?,?)");
  ...
  for(int i=0;i<quantity.length;i++){ 
   ...
   ps1.setInt(2,Integer.parseInt(medicines[i]));

   ps1.setInt(3,Integer.parseInt(quantity[i]));
     int e=ps1.executeUpdate();

        }
   pw.println(medicines);
...
pw.println("<body>");
  pw.println("<div class='container-fluid'>");
   ...
   pw.println("<form action='medical.html' method='GET'>");
    ...
    pw.println("</thead>");
      pw.println("<tbody>");
      for(int i=0;i<quantity.length;i++){
          PreparedStatement ps3=cn.prepareStatement("select mname from mlist where mid=?");
          ps3.setInt(1,Integer.parseInt(medicines[i]));
          PreparedStatement ps4=cn.prepareStatement("select mcost*quantity from medbill,mlist where medid=? and mid=? and medbid=?");
          ps4.setInt(1,Integer.parseInt(medicines[i]));
          ps4.setInt(2,Integer.parseInt(medicines[i]));
          ps4.setInt(3,b);
  ...
  ds1.next();
        ...
        pw.println("<tr>");
        pw.println("<td value='"+i+" '>"+(i+1)+"</td>");
        pw.println("<td value='"+medicines[i]+" '>"+ds.getString(1)+"</td>");
        pw.println("<td value='"+quantity[i]+" '>"+quantity[i]+" </td>");
        pw.println("<td value='2'>"+ds1.getInt(1)+"</td>");
      pw.println("</tr>");
        }
      pw.println("</tbody>");
    pw.println("</table>");
      pw.println("</form>");
       pw.println("<a href='medical.html'><button class='btn'>go back</button></a>");
pw.println("</body>");
pw.println("</html>");
